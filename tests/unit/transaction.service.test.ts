...
beforeEach(() => {
  ...
  txService = new TransactionService(store)
})

async function makeAcct(direction: 'debit' | 'credit', balance = 0) {
  return accountService.create({ direction, balance })
}
...
describe('TransactionService.create', () => {
  it('applies a balanced debit/credit transaction', async () => {
    ...
    const updatedDebit = await store.getAccount(debitAcct.id)
    const updatedCredit = await store.getAccount(creditAcct.id)
  ...
  it('decreases balance when directions differ', async () => {
    ...
    expect((await store.getAccount(debitAcct.id))!.balance).toBe(150)
    expect((await store.getAccount(creditAcct.id))!.balance).toBe(150)
  })
    ...
    await expect(
      txService.create({
        entries: [
          ...
          { account_id: 'non-existent-id', direction: 'credit', amount: 50 },
        ],
      }),
  ...
  it('throws 409 on duplicate transaction id', async () => {
    ...
    await expect(txService.create({ id, entries })).rejects.toMatchObject({ statusCode: 409 })
  })
